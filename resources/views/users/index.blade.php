        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="py-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
                ...
                <table class="w-full mx-auto divide-y table-auto">
                    <thead>
                        <tr>
                            <th class="px-8 py-6 text-lg text-left sm:px-10">User</th>
                            <th class="px-8 py-6 hidden lg:table-cell text-lg text-left sm:px-10">Email</th>
                            <th class="px-8 py-6 hidden lg:table-cell text-lg text-left sm:px-10">Role</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y">
                        @foreach ($users as $user)
                            <tr class="font-light text-violet-950">
                                <td class="table-cell px-8 py-4 sm:px-10">
                                    <a href="{{ route('users.show', [$user->id]) }}"
                                        class="hover:underline">{{ $user->name }}</a>
                                </td>
                                <td class="hidden lg:table-cell px-8 py-4 sm:px-10">
                                    <p> {{ $user->email }}</p>
                                </td>
                                <td class="hidden lg:table-cell px-8 py-4 sm:px-10">
                                ...
                                <td class="px-8 py-2 sm:px-10">
                                    <div class="flex justify-end">
                                        ...
                                        @endif
                                        @if (Auth::user()->id != $user->id && Auth::user()->hasPermission('users-delete'))
